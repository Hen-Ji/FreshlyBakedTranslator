...
WordType::WordType(QWidget *parent, int w, int h, int r)
    : QWidget{parent}
{
    resize(w, h);
    radius = r;
    ...
    text->resize(w, h);
    text->move(10, 0);
    text->setStyleSheet("font: 14pt 'Microsoft YaHei UI';");
}
...
void WordType::paintEvent(QPaintEvent *) {
    ...
    painter.drawRoundedRect(0, height()-2, width(), 2, radius, radius);
}
